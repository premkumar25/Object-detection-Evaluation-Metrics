    ...
    def GetPascalVOCMetrics(self,
        ...
        for c in classes:
            ...
            for d in range(len(dects)):
                ...
                # print('iou max', iouMax)
                for j in range(len(gt)):
                    # print('Ground truth gt => %s' % (gt[j][3],))
                    iou = Evaluator.iou(dects[d][3], gt[j][3])
                    # print(dects[d][3], gt[j][3])
                    # print(iou)
                    if iou > iouMax:
                        iouMax = iou
                        jmax = j
                    ...
                    # if iou <= 0.5:
                    #     print(dects[d])
                        # with open(os.path.join(r'D:\metrics\results_conf', 'iou.txt'), 'a') as f:
                    #         f.write('\n\niou: %s ' % str(iou))
                    #         f.write('\n%s' % str(dects[d]))
                        # print('iou', iou)
                        # print(dects[d])
                ...
                # - A detected "cat" is overlaped with a GT "cat" with IOU >= IOUThreshold.
                    ...
                    FP[d] = 1  # count as false positive
                    with open(os.path.join(r'D:\metrics\results', 'below_iou.txt'), 'a') as f:
                        iouMax = 0 if iouMax == 2.2250738585072014e-308 else iouMax
                        f.write('\niou: %s %s ' % (str(iouMax), str(dects[d])))
                    # print('iou', iou)
                    # print(dects[d])
                    # print("FP")
